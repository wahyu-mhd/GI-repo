...
export default function NewQuizPage(){
    ...
    const onSubmit = async (values: FormValues) => {
      ...
      try {
        ...
        if (normalizedAvailableFrom === null) throw new Error(t('startDateInvalid'))
        if (normalizedAvailableUntil === null) throw new Error(t('endDateInvalid'))
        if (
          ...
          throw new Error(t('endBeforeStart'))
        }
        ...
        if (!res.ok) throw new Error(t('saveQuizError'))
        router.push(`/teacher/courses/${courseId}/quizzes`, { locale })
      } catch (e) {
    ...
    if (!course) return <p>{t('courseNotFound')}</p>
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div key={idx} className="space-y-3 rounded border p-3">
                    ...
                    <FormLabel className="flex-1">{t('questionLabel', { index: idx + 1 })}</FormLabel>
                    <Select
                        value={q.type}
                        onValueChange={val => updateQuestion(idx, { type: val as QuestionType })}
                    >
                        <SelectTrigger><SelectValue placeholder={t('questionType')} /></SelectTrigger>
                        <SelectContent>
                        <SelectItem value="single">{t('multipleChoice')}</SelectItem>
                        <SelectItem value="multiple">{t('multipleSelect')}</SelectItem>
                        <SelectItem value="short">{t('shortAnswer')}</SelectItem>
                        <SelectItem value="long">{t('longAnswer')}</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
                ...
                <RichTextEditor
                    ...
                    <div className="space-y-2">
                        ...
                        <div key={cIdx} className="flex item-center gap-2">
                            ...
                            <Button type="button" variant="ghost" onClick={() => removeChoice(idx, cIdx)}>{t('remove')}</Button>
                        </div>
                        ))}
                        <Button type="button" variant="outline" onClick={() => addChoice(idx)}>+ {t('addOption')}</Button>
                </div>
                )}

                <div className="grid gap-3 md:grid-cols-3">
                  <div className="space-y-1">
                    <FormLabel>{t('pointsCorrect')}</FormLabel>
                    <Input
                      type="number"
                  ...
                  <div className="space-y-1">
                    <FormLabel>{t('pointsWrong')}</FormLabel>
                    <Input
                      type="number"
                  ...
                  <div className="space-y-1">
                    <FormLabel>{t('pointsSkip')}</FormLabel>
                    <Input
                      type="number"
                ...
                {(q.type === 'short' || q.type === 'long') && (
                    ...
                    <Button type="button" variant="ghost" onClick={() => removeQuestion(idx)}>{t('deleteQuestion')}</Button>
                </div>
                ))}
