...
export default function EditLessonPage() {
  ...
  const t = useTranslations('teacher.courseDetail.lessonEdit')
    ...
    <section className="max-w-2xl space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="space-y-1">
          <p className="text-sm font-medium">{t('moduleLabel')}</p>
          {modules.length > 0 ? (
            <select
              className="w-full rounded border px-3 py-2 text-sm"
              value={moduleId}
        ...
        <div className="space-y-2 rounded border p-3">
          <p className="text-xs font-medium text-slate-700">{t('createModuleHeading')}</p>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            >
              {creatingModule ? t('creatingModule') : t('addModule')}
            </button>
          </div>
        </div>
        <div className="space-y-1">
          <p className="text-sm font-medium">{t('contentLabel')}</p>
          <RichTextEditor
            value={content}
            onChange={setContent}
      ...
      <div className="rounded-lg border bg-white p-3 shadow-sm">
        <p className="text-xs font-semibold text-slate-500 mb-1">{t('preview')}</p>
